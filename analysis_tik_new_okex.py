...
for i in range(M, len(askPrice)):
    ...
    open_flag =  abs(spreadPrice[i]) < MAX_SPREAD and closePrice[i-1] - closePrice[i-2] > 0  and 1.0 * (closePrice[i] - closePrice[i-1]) / (closePrice[i-1] - closePrice[i-2]) > 0 \
                 and np.std(closePrice[i-M:i-1]) < 4.5 and np.std(closePrice[i-M:i-1]) > 0\
    ...
    short_flag = abs(spreadPrice[i]) < MAX_SPREAD and closePrice[i-1] - closePrice[i-2] < 0 and 1.0 * (closePrice[i] - closePrice[i-1]) / (closePrice[i-1] - closePrice[i-2]) > 0 \
                 and np.std(closePrice[i-M:i-1]) < 4.5 and np.std(closePrice[i-M:i-1]) > 0 \

...
from utils.array_manager import VtTickData
from utils.array_manager import BarManager
...
while i < len(closePrice)- 100:
    ...
    open_flag = abs(spreadPrice[i-1]) < MAX_SPREAD and abs(closePrice[i-1] - closePrice[i-2]) < 2 \
                and closePrice[i] - closePrice[i-1] > 0 and abs(closePrice[i] - closePrice[i - 1]) < 2 \
                ...
                # and (closePrice[i] > np.max(closePrice[i-300:i]))
                #and np.mean(closePrice[i-1000:i]) > np.mean(closePrice[i-4000:i])

                #

    #open_flag = False

    short_flag = abs(spreadPrice[i-1]) < MAX_SPREAD and abs(closePrice[i-1] - closePrice[i-2]) < 2 \
                 and closePrice[i] - closePrice[i-1] < 0 and abs(closePrice[i] - closePrice[i - 1]) < 2 \
                 and (closePrice[i - 2] - np.max(closePrice[i - N:i - 2])) / np.max(closePrice[i - N:i - 2]) > -DOWN_TREND \
                 and (closePrice[i - 1] - np.max(closePrice[i - N:i - 2])) / np.max(closePrice[i - N:i - 2]) < -DOWN_TREND \
